...
export default function LandingTemplate({ completion }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formJson.location.length > 20)
      return setError('Destination must be less than 20 characters')
    setLoading(true)
    ...
    <div className={styles.contentContainer}>
      ...
      <div className={styles.center}>
        {loading ? (
          ...
          <h1 className="text-4xl leading-normal md:text-6xl md:leading-[5rem] ">
            {customMessage}
          </h1>
        )}
      </div>
      <div className={styles.inputContainer}>
        <form onSubmit={handleSubmit} className="justify-center flex">
          <label>
            <input
              className="text-gray-800 bg-white pr-2 w-full"
              name="location"
          ...
          <button
            className={
              'bg-blue-500 px-6 rounded-full font-bold text-white flex hover:bg-blue-800 disabled:bg-gray-300 items-center ml-2 min-w-fit'
            }
            type="submit"
            disabled={loading}
          >
            {' '}
            {loading && <Spinner />} {loading ? 'Generating...' : 'Inspire me'}
          </button>
        </form>
