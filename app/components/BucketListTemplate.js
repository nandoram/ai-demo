...
export default function BucketlistTemplate({}) {
  ...
  const [questionsVal, setQuestionsVal] = useState({
    adventure: '5',
    social: '5',
    nature: '5',
    creativity: '5',
  ...
  const renderQuestions = () => {
    return questions.map((data, i) => (
      <div key={i} className="mb-16">
        ...
        <Slider
          min={10}
          handleStyle={{ borderColor: '#638ccd' }}
          activeDotStyle={{ borderColor: '#9bb5ff' }}
          trackStyle={{ backgroundColor: '#638ccd' }}
          defaultValue={50}
          marks={{
            10: 1,
            20: 2,
          ...
          step={null}
          onChange={(val) => setValues(val.toString().slice(0, -1), data.type)}
        />
      </div>
    ))
  }
  ...
  const renderBucketList = () => {
    ...
    if (!Array.isArray(bucketList) && bucketList.length > 1)
      return (
        <li className="py-8 pl-2 text-lg font-normal leading-6 text-gray-900">
          {bucketList}
        </li>
      )
    return bucketList.map((data, i) => {
      if (data.length > 0 && !filterWord.test(data)) {
        return (
          <li
            key={i}
            className="py-8 pl-2 text-lg font-normal leading-6 text-gray-900"
          >
            {data}
          </li>
        )
    ...
    if (res) {
      ...
      if (data) {
        const questionsAndAnswers = {
          ...
          cultural: answers.cultural,
          learning: answers.learning,
          physical: answers.physical,
          food: answers.food,
          historical: answers.historical,
          family: answers.family,
        }
        await fetch(
          'https://dpojactvu7.execute-api.us-east-1.amazonaws.com/addBucketList',
          {
            method: 'POST',
            body: JSON.stringify({
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="justify-center flex flex-col">
        <button
          className="bg-blue-500 py-4 px-6 rounded-full font-bold text-white flex self-center hover:bg-blue-800 disabled:bg-gray-300"
          type="submit"
          disabled={loading || bucketList}
        >
          {' '}
          {loading && <Spinner />}{' '}
          {loading ? 'Generating...' : 'Generate Bucket List'}
        </button>
        <Link
          className="text-gray-500 text-center mt-4 font-bold underline hover:text-gray-900"
          href="/demos/bucketlist/view-all"
        >
          View All
        </Link>
      </form>
