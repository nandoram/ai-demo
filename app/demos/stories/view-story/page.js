...
export default function Story({}) {
    ...
    return Object.keys(questionsAndAnswers).map((data, i) => {
      if (data.length > 0 && data !== 'lesson') {
        return (
          <li
            key={i}
            className="py-1  flex flex-col justify-between px-2 text-xs capitalize leading-6 text-gray-900"
          >
            <span className=" font-medium ">{data}</span>
            <span className=" font-black flex self-center">
  ...
  return (
    <div className={'p-6 flex flex-col'}>
      ...
      {story ? (
        <>
          <h2 className="text-gray-900 font-bold capitalize text-center">
            {questionsAndAnswers.lesson}
          </h2>
          ...
          <p className=" text-gray-900 mt-8 self-center max-w-4xl leading-loose z-10 relative md:px-8 md:mx-8 border rounded-[20px] border-gray-200 p-5">
            {story}
          </p>
        </>
      ) : (
