...
const generateAndAddStory = async (metaData) => {
  ...
  try {
    res = await fetch(
      'https://dpojactvu7.execute-api.us-east-1.amazonaws.com/kid-stories/createStory',
      {
        method: 'POST',
        body: JSON.stringify({
          content: `Generate a short ${answers.category} children story for a ${answers.age} year old ${genderString} named ${answers.name} that is learning a lesson about ${answers.lesson}`,
        }),
      },
    )
  } catch (e) {
    return {
      data: null,
      error:
        'There was a timeout error generating your story. Please try again.',
    }
  }
  if (res) {
    try {
    ...
    if (data) {
      ...
      await fetch(
        'https://dpojactvu7.execute-api.us-east-1.amazonaws.com/kid-stories/addStory',
        {
          method: 'POST',
          body: JSON.stringify({
